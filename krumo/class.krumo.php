...
This is a list of all the values from the <code><b><?php echo realpath($ini_file) ? realpath($ini_file) : $ini_file;?></b></code> INI file.
	...
	*/
	Public Static Function dump($data) {
		...
		while($d = array_pop($_)) {
			if($d['file'])
				$trace[] = array("file" => $d['file'],"line" => $d['line']);
	...
	<ul class="krumo-node krumo-first">
		...
		<li class="krumo-footnote">
			<div class="krumo-version" style="white-space:nowrap;">
				<h6>Hikari Krumo version <?php echo krumo::version();?></h6> | <a
					href="http://Hikari.ws/krumo/">http://Hikari.ws/krumo/</a>
			</div>
		
<?php
$traced = count($trace)-1;
if($traced>0 && strpos($trace[$traced]['file'],"hikari-tools.php")) $traced--;

		if (@$trace[$traced]['file']) { ?>
		<span class="krumo-call" style="white-space:nowrap;">
			Called from <code><?php echo $trace[$traced]['file']?></code>,
				line <code><?php echo $trace[$traced]['line']?></code></span>
		<?php } ?>
		&nbsp;
		</li>
	</ul>
	...
	*/
	Private Static Function _config($group, $name, $fallback=null) {
		...
		if (empty($_config)) {
			...
			$_config = array(
				"skin"	=> array("selected" => $hkKrumo_Op->optionsDBValue['skin']),
				"css"	=> array("url" => plugin_dir_url(__FILE__))
			);
			
			$_config = apply_filters('wpkrumo_config', $_config);
...
<?php
	...
	*/
	...
	public static function isKrumoEnabled(){
		return krumo::_debug();
	}
...
<li class="krumo-child">
	...
	<div class="krumo-nest" style="display:none;">
		<ul class="krumo-node">
			...
			<li class="krumo-child">
				<?php //echo HkKrumo_Op::formatCode($data); ?>
				<pre class="krumo-preview"><?php echo htmlSpecialChars($data);?></pre>
			</li>
